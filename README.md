...
| [codex-model-routing](codex-model-routing/) | Choosing a Codex-selectable GPT-5.6 Sol/Terra/Luna model and reasoning effort by expected cost to correct, verified completion. Uses dated Codex coding-agent and general-task cost-per-task evidence, accounts for stakes, reversibility, verification, context, caching, and escalation, and enforces a hard four-agent delegation cap including guarded `ultra` use. | Picking a Codex model or effort, deciding whether to escalate or down-route, assigning a Codex worker, controlling model-routing cost, or preventing runaway recursive subagents. |

...
| [codex-model-routing](codex-model-routing/) | Choosing a Codex-selectable GPT-5.6 Sol/Terra/Luna or GPT-5.4 Mini configuration by expected cost to correct, verified completion. Uses a task matrix and dated configuration-level cost, quality, runtime, and token evidence; treats Luna as the cost center, Terra as a runtime/token or `max` bridge, Sol as the judgment ceiling, and enforces a hard four-agent delegation cap including guarded `ultra` use. | Picking a Codex model or effort, deciding whether to escalate or down-route, assigning a bounded Mini worker, controlling model-routing cost, or preventing runaway recursive subagents. |
